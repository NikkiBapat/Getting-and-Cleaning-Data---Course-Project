...
## first install the 'plyr' library needed for the ddply function
install.packages ("plyr")
library (plyr)

# We will take means of Col 1- 66 and skip 67 & 68 columns as last two are (activity & subject)
averages_data <- ddply(all_data, .(subject, activity), function(x) colMeans(x[, 1:66]))

write.table(averages_data, "averages_data.txt", row.name=FALSE)

## average_data.txt file now should be present in the working directory
